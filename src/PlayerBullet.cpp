...
void ShooterManager::Init(PlayerData* player, int& powerm, int& powers, float& hitbox, float& grazebox, float& itembox, float& spdnf, float& spdf, int& maxdmg)
{
    ...
    while (i < optioncount)
    {
        ...
        for (int j = 0; j < p; j++)
        {
            ...
            if (max-powers >= powerm) options[i].anm2 = AnmManager::newAnim(player->OptionAnm(true));
            i++;
            off+=8;
        }
        p++;
    ...
    int s = 0;
    int optionoffset = 0;
    for (int i = 0; i < 2 * (pwr_lvl_cnt+1); i++)
    {
        intRange rg = {(i%(pwr_lvl_cnt+1))*powerstep, (i%(pwr_lvl_cnt+1)+1)*powerstep};
        if (rg.min/powerstep == 0) optionoffset = 0;
        else optionoffset += (rg.min/powerstep-1);
        bool focus = floor((float)i/(float)(pwr_lvl_cnt+1));
        bool check = false;
        while (!check)
        {
            if (s >= shootercount) break;
            ...
            if (shooters[s].option != 0) shooters[s].option += optionoffset;
...
void ShooterManager::Update(bool shooting, bool focus, float framespeed)
{
    ...
    {
        if (focus && options[i].x != options[i].fx) options[i].x += framespeed*(options[i].fx - options[i].nfx)/OPTION_MOVE_TIME;
        if (focus && options[i].y != options[i].fy) options[i].y += framespeed*(options[i].fy - options[i].nfy)/OPTION_MOVE_TIME;
        if (!focus && options[i].x != options[i].nfx) options[i].x += framespeed*(options[i].nfx - options[i].fx)/OPTION_MOVE_TIME;
        if (!focus && options[i].y != options[i].nfy) options[i].y += framespeed*(options[i].nfy - options[i].fy)/OPTION_MOVE_TIME;
        if (options[i].powermax > power && options[i].powermin <= power )
        {
            ...
            if (!options[i].isDrawn)
            {
                ...
                if (options[i].anm2 != -1) AnmManager::interrupt(options[i].anm2,2);
            }
        }
        else if (options[i].isDrawn)
        {
            ...
            if (options[i].anm2 != -1) AnmManager::interrupt(options[i].anm2,3);
        }
    }
    ...
    if (shooting)
    {
        ...
        {
            if ((focus && !shootersFocus[i]) || (!focus && shootersFocus[i])) continue;
            if (power < shootersPower[i].min || power >= shootersPower[i].max) continue;
            if (((int)floor(shootCounter) % shooters[i].fire_rate) != shooters[i].start_delay) continue;
            //if ((shootCounter - shooters[i].start_delay) < 0 || ((int)floor(shootCounter) % shooters[i].fire_rate) != 0) continue;
            int qqq = 0; while (qqq++ < bulletcount && bullets[current_pos].active) current_pos = (current_pos+1)%bulletcount;
