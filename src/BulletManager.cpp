...
void BulletManager::DestroyHandler(unsigned int& i)
{
    if (i == handlers.size() - 1) {
        delete handlers[i];
        handlers[i] = nullptr;
        while (handlers[i] == nullptr) {
            handlers.pop_back();
            i--;
        }
    } else if (i < handlers.size() && handlers[i] != nullptr) {
        delete handlers[i];
        handlers[i] = nullptr;
    }
    i = -1;
...
void BulletManager::Transform::Apply(Bullet* bu)
{
    ...
    case EX_ANIM:
        if (a > 0)
            AnmManager::interrupt(bu->anm, 6 + a);
        break;
    case EX_ACCEL_F:
        bu->accel = r;
        bu->angleaccel = 0;
...
{
    if (frame < saccelframe)
        speed += accel;
    if (frame < aaccelframe)
        direction += angleaccel;
    if (frame != 0) {
        x += math::lengthdir_x(speed, direction);
        y += math::lengthdir_y(speed, direction);
    }
    if (anm != -1)
        AnmManager::UpdateChild(anm, x, y, 0, 0, 0, direction - PI1_2, 1, 1, true);
    if (x < -200 || x > 200 || y < -300 || y > 300) {
        if (!dontdespawn && frame < 500) {
            if (anm != -1)
                AnmManager::deleteAnim(anm);
            if (box != -1)
                CollisionManager::destroyEnemyHitbox(box);
            Reset();
        }
    }
    frame++;
    if (box != -1)
        CollisionManager::MoveBox(0, box, x, y);
}
#include "Pickup.h"
void BulletManager::Bullet::Delete(bool itemspawn)
{
    if (itemspawn)
        Pickup::SpawnCancelPickup(x, y);
    ...
    if (mode < 9)
        for (int j = 0; j < ct1; j++) {
            for (float i = 0; i < ct2; i++) {
                ...
                if (mode == AIM_RAND_RING || mode == AIM_MEEK)
                    s = spd1 + (spd2 - spd1) * Random::Float01();
        ...
        for (int i = 0; i < ct1; i++) {
            for (int j = 0; j < ct2 / 2 + 1; j++) {
                ...
                if (j != 0)
                    BulletManager::CreateBullet(id, x, y, s, a2);
            }
        }
...
{
    int i = 0;
    while (bullet_pool[current_pos].active && i < BH_MAX_BULLET) {
        current_pos = (current_pos + 1) % BH_MAX_BULLET;
        i++;
    }
    if (bullet_pool[current_pos].active)
        bullet_pool[current_pos].Delete(false);
