...
{
    for (int i = 0; i < digit; i++)
        AnmManager::deleteAnim(anms[i]);
}

void textAnm::interrupt(int i)
{
    for (int j = 0; j < digit; j++)
        AnmManager::interrupt(anms[j], i);
}
void textAnm::reset()
{
    int n = num;
...
void textAnm::set_number(int num)
{
    ...
    for (int i = digit - 1; i >= 0; i--) {
        if (!showAll)
            AnmManager::UpdateChild(anms[i], (float)(offset + space * i), 0, 0, 0, 0, 0, num != 0, num != 0, 1);
...
void AsciiElement::set_text(std::string const& t)
{
    text = t;
}
...
AsciiElement* AsciiManager::newTxt(int l, int f, std::string const& t)
{
    int i = 0;
    while (asciipool[currentpos].isActive() && i < 100) {
