...
class THAnim {
public:
    THAnim(void (*anmC)(AnimScript*), void (*anmCL)(AnimScript*), void (*anmLC)(AnimScript*), void (*anmCR)(AnimScript*), void (*anmRC)(AnimScript*), void (*anmSP)(AnimScript*) = nullptr)
    {
        this->anmC = anmC;
        this->anmCL = anmCL;
        this->anmCR = anmCR;
        ...
        this->anmRC = anmRC;
        this->anmSP = anmSP;
        anm = AnmManager::newAnim(anmC, -1, 1);
    }

    void PlayAnimSP()
    {
        if (anmSP != nullptr)
            AnmManager::chgScript(anm, anmSP, 1);
        AnmManager::UpdateChild(anm, x, y, 0, 0, 0, 0, 1, 1, true);
    }
    void Reset()
    {
        if (anm != -1 && AnmManager::IsActive(anm))
            AnmManager::chgScript(anm, anmC, 1);
