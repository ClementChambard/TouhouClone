...
{
    if (Instance == nullptr) Instance = new PlayerEntity(player!=-1?player:0);
    return Instance;
}
...
void PlayerEntity::Collect(int PickupType, float ywhencollected)
{
    ...
    if (power_before != power)
    {
        ...
        if (power_before < maxpower && power >= maxpower) AnmManager::newAnim(frontFullPower);
...
void PlayerEntity::Update(float framespeed)
{
    ...
    if (!focus && lshift) anmSlow = AnmManager::newAnim(effectSlow2);
    else if (focus && !lshift) { AnmManager::interrupt(anmSlow,1); anmSlow = -1; }
    focus = lshift;
    ...
    x += floor(hsp);
    y += floor(vsp);
    if (x > PLAYER_BORDER_RIGHT) { x = PLAYER_BORDER_RIGHT; hsp = 0; xdir = 0; } 
    if (x < PLAYER_BORDER_LEFT) { x = PLAYER_BORDER_LEFT; hsp = 0; xdir = 0; } 
    if (y > PLAYER_BORDER_TOP) { y = PLAYER_BORDER_TOP; vsp = 0; } 
    if (y < PLAYER_BORDER_BOTTOM) { y = PLAYER_BORDER_BOTTOM; vsp = 0; } 
    hsp_frac = hsp-floor(hsp);
    vsp_frac = vsp-floor(vsp);

    if (y > PLAYER_COLLECTION_LINE) Pickup::Collect_All();

    /////   ANIMATION   /////
    anm->Update(xdir,x,y);
    if (anmSlow != -1) AnmManager::UpdateChild(anmSlow,x,y,0,0,0,0,1,1,true);
    }
    ...
    if (deadframe == 8)
    {
        ...
        x = 0;y = -240;
        invFrame = 240;
        if (life < 0) PauseMenu::Open(1);
    }
    else if (deadframe == 28) anm->Reset();
    else if (deadframe > 28 && deadframe < 68) { y+=1.5f; anm->Update(0,x,y); BulletManager::etClear(); }
