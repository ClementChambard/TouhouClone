...
void PauseMenu::Open(int i)
{
    ...
    opened = true;
    firstupdate = true;
    NSEngine::engineData::gameflags |= 0b10000000;
...
{
    opened = false;
    NSEngine::engineData::gameflags &= 0b01111111;
    AnmManager::interrupt(anmback,1);
    AnmManager::interrupt(anmtitle,1);
    for (option o : options) o.del();
    options.clear();
}
...
void PauseMenu::Update()
{
    ...
    else if (KEYPRESSED(K_SHOOT) || KEYPRESSED(K_CONFIRM)) options[optionOn].click();
    if (Inputs::Keyboard().Pressed(NSK_escape)) Close();
}
...
void PauseMenu::conf::Open(std::function<void(void)> funct)
{
    ...
    for (option o : options) AnmManager::interrupt(o.anm,5);
}
