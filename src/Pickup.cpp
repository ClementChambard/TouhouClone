...
void Pickup::Collect_All()
{
    ...
    for (int i = 0; i < 500; i++)
        if (PickupPool[i].active && !PickupPool[i].collected) {
...
void Pickup::Collect_All(int type)
{
    PlayerEntity* player = PlayerEntity::GetInstance();
    for (int i = 0; i < 500; i++)
        if (PickupPool[i].active && PickupPool[i].type == type && !PickupPool[i].collected) {
            PickupPool[i].collected = true;
            PickupPool[i].ywhencollected = player->y;
            PickupPool[i].spd = 0;
            PickupPool[i].xstep = 0;
...
void Pickup::Pupdate(float framespeed)
{
    ...
    if (type == 50) {
        PlayerHUD::itemc++;
        ywhencollected++;
        if (ywhencollected == 20)
            collected = true;
    } else
        PlayerHUD::item++;
    if ((type == 0 || type == 3) && player->isMaxPower())
        type = 7;
    ...
    if (collected) {
        ...
        if (spd > PICKUP_COLLECTED_MAX_SPEED)
            spd = PICKUP_COLLECTED_MAX_SPEED;
        ...
        if (y > yto - gravityheight)
            spd += PICKUP_ACCEL;
        if (spd < -PICKUP_MAX_SPEED + 1)
            spd = -PICKUP_MAX_SPEED + 1;
        y += spd;
        if (abs(xto - x) > abs(xstep))
            x += xstep;
    ...
    if (y < 228)
        AnmManager::UpdateChild(anm, x, y, 0, 0, 0, 0, 1, 1, true, yy >= 228 ? 2 : 0);
    else
        AnmManager::UpdateChild(anm, x, type < 2 ? 216.f : 208.f, 0, 0, 0, 0, 1, 1, true, yy < 228 ? 3 : 0);
}
...
void Pickup::CreateCollectText(std::string const& text, float x, float y, int r, int g, int b)
{
