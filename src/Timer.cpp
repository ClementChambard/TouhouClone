...
{
    if (!b)
        if (nb1 != -1 && nb2 != -1 && nb3 != -1 && nb4 != -1 && dot != -1) {
...
void Timer::ShowTimeScore(float time, float bestTime)
{
    ...
    int n1 = AnmManager::newAnim(asciiNum2, -1, 1);
    AnmManager::anim(n1)->offset_sprite((int)floor(time) % 10);
    AnmManager::UpdateChild(n1, xx, yy1, 0, 0, 0, 0, 1, 1, 1);
    int n2 = AnmManager::newAnim(asciiNum1, -1, 1);
    AnmManager::anim(n2)->offset_sprite((int)floor(time / 10.f) % 10);
    AnmManager::UpdateChild(n2, xx, yy1, 0, 0, 0, 0, 1, 1, 1);
    int n3 = AnmManager::newAnim(asciiNum1, -1, 1);
    AnmManager::anim(n3)->offset_sprite((int)floor(time * 10) % 10);
    AnmManager::UpdateChild(n3, xx + 20 - 50, yy1 - 4 + 100, 0, 0, 0, 0, .5, .5, 1);
    int n4 = AnmManager::newAnim(asciiNum2, -1, 1);
    AnmManager::anim(n4)->offset_sprite((int)floor(time * 100) % 10);
    AnmManager::UpdateChild(n4, xx + 20 - 50, yy1 - 4 + 100, 0, 0, 0, 0, .5, .5, 1);
    int d = AnmManager::newAnim(asciiNum1, -1, 1);
    AnmManager::anim(d)->offset_sprite(11);
    AnmManager::UpdateChild(d, xx + 22, yy1, 0, 0, 0, 0, 1, 1, 1);
    int s = AnmManager::newAnim(asciiNum1, -1, 1);
    AnmManager::anim(s)->offset_sprite(12);
    AnmManager::UpdateChild(s, xx + 32 - 50, yy1 - 4 + 100, 0, 0, 0, 0, .5, .5, 1);
    AnmManager::interrupt(n1, 10);
    AnmManager::interrupt(n2, 10);
    AnmManager::interrupt(n3, 10);
    AnmManager::interrupt(n4, 10);
    AnmManager::interrupt(d, 10);
    AnmManager::interrupt(s, 10);
    n1 = AnmManager::newAnim(asciiNum2, -1, 1);
    AnmManager::anim(n1)->offset_sprite((int)floor(bestTime) % 10);
    AnmManager::UpdateChild(n1, xx, yy2, 0, 0, 0, 0, 1, 1, 1);
    AnmManager::anim(n1)->set_color(128, 128, 128);
    n2 = AnmManager::newAnim(asciiNum1, -1, 1);
    AnmManager::anim(n2)->offset_sprite((int)floor(bestTime / 10.f) % 10);
    AnmManager::UpdateChild(n2, xx, yy2, 0, 0, 0, 0, 1, 1, 1);
    AnmManager::anim(n2)->set_color(128, 128, 128);
    n3 = AnmManager::newAnim(asciiNum1, -1, 1);
    AnmManager::anim(n3)->offset_sprite((int)floor(bestTime * 10) % 10);
    AnmManager::UpdateChild(n3, xx + 20 - 50, yy2 - 4 + 100, 0, 0, 0, 0, .5, .5, 1);
    AnmManager::anim(n3)->set_color(128, 128, 128);
    n4 = AnmManager::newAnim(asciiNum2, -1, 1);
    AnmManager::anim(n4)->offset_sprite((int)floor(bestTime * 100) % 10);
    AnmManager::UpdateChild(n4, xx + 20 - 50, yy2 - 4 + 100, 0, 0, 0, 0, .5, .5, 1);
    AnmManager::anim(n4)->set_color(128, 128, 128);
    d = AnmManager::newAnim(asciiNum1, -1, 1);
    AnmManager::anim(d)->offset_sprite(11);
    AnmManager::UpdateChild(d, xx + 22, yy2, 0, 0, 0, 0, 1, 1, 1);
    AnmManager::anim(d)->set_color(128, 128, 128);
    s = AnmManager::newAnim(asciiNum1, -1, 1);
    AnmManager::anim(s)->offset_sprite(12);
    AnmManager::UpdateChild(s, xx + 32 - 50, yy2 - 4 + 100, 0, 0, 0, 0, .5, .5, 1);
    AnmManager::anim(s)->set_color(128, 128, 128);
    AnmManager::interrupt(n1, 10);
    AnmManager::interrupt(n2, 10);
    AnmManager::interrupt(n3, 10);
    AnmManager::interrupt(n4, 10);
...
void Timer::Update()
{
    ...
    if (time > 0) {
        ...
        if (PlayerEntity::GetInstance()->y > 120) {
            if (!fade) {
                interruptAll(5);
                fade = true;
            }
        ...
        if (floor(timePrevious) == floor(time))
            return;
        if (time == 0)
            interruptAll(7);
        if (time <= 2) {
