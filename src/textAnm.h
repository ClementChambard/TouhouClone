...
public:
    ...
    ~textAnm();
    textAnm(textAnm const&) = delete;
    textAnm& operator=(textAnm const&) = delete;
...
class AsciiElement {
public:
    void Init(int l, int f, std::string const& t)
    {
        destroy();
        font = f;
        active = 1;
    ...
    void destroy()
    {
        ...
        col = glm::vec<3, uint8_t>(255, 255, 255);
        a = 255;
        layerID = 0;
        active = 0; /*halign=NSEngine::fa_left;valign=NSEngine::fa_top;*/
        text = "";
    }
...
public:
    ...
    static AsciiElement* newTxt(int l, int f, std::string const& t);
    static void Update() { }
    static void Draw()
    {
        for (int i = 0; i < 100; i++)
