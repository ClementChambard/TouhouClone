...
{
    if (sub_pool != nullptr) delete[] sub_pool;
...
void ECLManager::Update()
{
    for (int i = 0; i < 200; i++)
    {
        if (!sub_pool[i].active) continue;
        sub_pool[i].Update();
    }
    if (boss != nullptr) thebossanms.Update();
}
...
void ECLS::Update(bool child)
{
    ...
    sub(this);
    frame++;
    if (endat != nullptr && !child) endat->Update();
}

void ECLS::Delete()
{
    active = false;
    if (asyncchilds != nullptr) { delete asyncchilds; asyncchilds = nullptr; }
    if (endat != nullptr) { delete endat; endat = nullptr; }
}
...
int ECLManager::newSub(ECLSub_t sub)
{
    ...
    int i = 0; while(sub_pool[pos].active && i++ < 200) pos = (pos+1)%200; i = pos; pos = (pos+1)%200;
...
{
    if (boss == nullptr) return enmCreate(sub,x,y,hp,score,item);
    return -1;
}
int ECLManager::enmCreateFM(ECLSub_t sub, float x, float y, float hp, int score, int item)
{
    if (boss == nullptr) return enmCreateM(sub,x,y,hp,score,item);
    return -1;
}

void ECLManager::deleteSub(int i)
{
    if (i < 0 || i >= 200 || !sub_pool[i].active) return;
    sub_pool[i].Delete();
}

ECLManager::ECLSub* ECLManager::Sub(int i)
{
    if (i < 0 || i >= 200 || !sub_pool[i].active) return nullptr;
    return &(sub_pool[i]);
}
...
{
    AnmManager::interrupt(bossbar1,1); bossbar1 = -1;
    AnmManager::interrupt(bossbar2,1); bossbar2 = -1;
    AnmManager::interrupt(bossbar3,1); bossbar3 = -1;
...
{
    lifeDisplayed = abs(boss->life-lifeDisplayed) < 100 ? boss->life :
        (lifeDisplayed < boss->life ? lifeDisplayed+100 : lifeDisplayed-100);
    ...
    int interrupt = 0;
    if (fadin && !playerin) {fadin = false; interrupt = 2; }
    if (!fadin && playerin) {fadin = true; interrupt = 3; }
...
void ECLManager::bossAnms::AddSection(float life)
{
    ...
    AnmManager::anim(bossbarsections.back())->set_rot(0,0,a); a += PI1_2;
...
{
    for (int s : bossbarsections) if (s != -1) AnmManager::interrupt(s,1);
...
void ECLManager::enmKillAll()
{
    for (int i = 0; i < 200; i++)
    {
        if (sub_pool[i].active && sub_pool[i].endat != nullptr)
            if (!(sub_pool[i].flags&128 || sub_pool[i].flags&32 || sub_pool[i].flags&1024)) sub_pool[i].Delete();
    }
}
