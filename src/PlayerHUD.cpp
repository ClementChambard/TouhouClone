...
{
    PlayerHUD::hiscore = hiscore;
    this->maxpower = maxpower;
    this->steppower = steppower;
    AnmManager::newAnim(frontAll);
    int anm = AnmManager::newAnim(frontDifficulty1, -1, 1);
    AnmManager::anim(anm)->offset_sprite(ECLManager::difficulty);
    anm = AnmManager::newAnim(frontDifficulty2, -1, 1);
    AnmManager::anim(anm)->offset_sprite(ECLManager::difficulty);
    for (int i = 0; i < 8; i++) {
...
void ScoreText()
{
    ...
    int augment = 1000;
    if (PlayerHUD::score - score > 10000)
        augment *= 10;
    if (PlayerHUD::score - score > 100000)
        augment *= 10;
    ...
    stringScoreCommas(scores);
    stringScoreCommas(hiscores);
...
void PlayerHUD::Update(float power, int score, int life, int spell, int pointValue, int graze, float framespeed)
{
    ...
    int score_before = PlayerHUD::score;
    PlayerHUD::score = score;
    this->power = power;
    this->pointValue = pointValue;
    this->graze = graze;
    if (score_before <= hiscore && score > hiscore)
        AnmManager::newAnim(frontHighScore);
}
